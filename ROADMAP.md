...
- Hash-chained traces with optional HMAC sealing shipped in commit cb6168c;
  key ids + rotation (#80) and Ed25519 public-key signatures (#81) followed.
...
- Versioned JSON Schemas + format-version enforcement for the plan and witness
  artifacts (#17, #38); unified witness envelope (#108); `trace.py` extraction
  with a shared event vocabulary (#67).
...
6. Plan-level compiler optimizations for token, latency, and risk (#106)
